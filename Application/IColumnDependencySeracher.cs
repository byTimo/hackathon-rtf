...
namespace Application
{
    ...
    public class ColumnDependencySeracher : IColumnDependencySeracher
    {
        private static readonly ColumnImpact lol = new ColumnImpact(EDbVendor.dbvoracle, true, false, false, false);
        
        public Dictionary<ColumnInfo, HashSet<ColumnInfo>> Search(string query)
        {
            lol.Impact(query);
            var dependency = lol.ImpactResult;
            
            if(dependency.StartsWith("syntax error"))
            {
                return null;
            }

            var lines = new Regex("\r\n").Split(dependency);
            var result = new Dictionary<ColumnInfo, HashSet<ColumnInfo>>();
            ...
            foreach (var line in lines)
            {
                ...
                var a = line.Substring(0, line.IndexOf(" ")).Split('.');
                var columnInfo = a.Length == 2 ? new ColumnInfo(a[1], a[0]) : new ColumnInfo(a[0], null);
                result[columnInfo] = new HashSet<ColumnInfo>();
                
                foreach (var column in line.Substring(line.IndexOf(':') + 1).Split(new []{", "}, StringSplitOptions.RemoveEmptyEntries))
                {
                    var b = column.Split('.');
                    result[columnInfo].Add(b.Length == 2 ? new ColumnInfo(b[1], b[0]) : new ColumnInfo(b[0], null));
                }
            }
        ...
        public class ColumnImpact
        {
            public class LinkedHashMap<T, U>
            {
                ...
                public Dictionary<T, LinkedListNode<Tuple<U, T>>>.KeyCollection Keys { get { return D.Keys; } }
            }
            ...
            public virtual bool CollectColumnInfo { get { return collectColumnInfo; } set { this.collectColumnInfo = value; } }
            ...
            public ColumnImpact(EDbVendor dbVendor, bool simply, bool isXML, bool isColumnLevel, bool traceView)
            {
                if (traceView)
                {
                    this.traceView = true;
            ...
            private bool findColumnInTables(TColumn column, string tableName, TCustomSqlStatement select, int level, ClauseType clause)
            {
                ...
                if (tables.size() == 1)
                {
                    ...
                    if ((lzTable.TableType == ETableSource.objectname) && (string.ReferenceEquals(tableName, null) ||
                                                                           (!string.ReferenceEquals(tableName, null) && lzTable.AliasClause == null && getTableName(lzTable).Equals(tableName, StringComparison.OrdinalIgnoreCase)) ||
                                                                           (!string.ReferenceEquals(tableName, null) && lzTable.AliasClause != null && lzTable.AliasClause.ToString().Equals(tableName, StringComparison.OrdinalIgnoreCase))))
                    {
                        ret = true;
            ...
            public virtual string ImpactResult { get { return buffer.ToString(); } }

            public virtual IList<TColumn> ColumnInfos { get { return columnCollection; } }

            public virtual bool Impact(string query)
            {
                sqlparser.sqltext = query ?? sqlparser.sqltext;
                ...
                switch (field.Expr.ExpressionType)
                {
                    ...
                    default:
                        ...
                        if (field.Expr.ExpressionType == EExpressionType.function_t)
                        {
                            ...
                            if (func.FunctionName.ToString().Equals("count", StringComparison.OrdinalIgnoreCase) || func.FunctionName.ToString().Equals("sum", StringComparison.OrdinalIgnoreCase) ||
                                func.FunctionName.ToString().Equals("row_number", StringComparison.OrdinalIgnoreCase))
                            {
                                if (!simply)
                                {
                                    ...
                                    for (int i = 0; i < select.tables.size(); i++)
                                    {
                                        ...
                                        {
                                            buffer.Append(buildString(" ", level + 1) + "--> table " + select.tables.getTable(i).ToString() +
                                                          (select.tables.getTable(i).AliasClause != null ? (" " + select.tables.getTable(i).AliasClause.ToString()) : "") + "\r\n");
                                        }
                                    }
                                }
